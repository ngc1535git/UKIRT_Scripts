...
class Clicker(object):
    ...
    def __init__(self, master, zoom, filelist):
        ...
        self.reject_index = [False for i in range(len(self.files))]
        self.index = 0
        self.images = []
        self.ldac = []
        self.mjd = []
        ...
        # create big red x for rejected images
        self.rejectX_id = self.canvas.create_text(-100, -100, text="X", fill='red', justify=CENTER, font=tkFont.Font(family='Helvetica', size=96, weight='bold'))
    ...
    def toggleRejectFrame(self):
        self.reject_index[self.index] = not self.reject_index[self.index]
    ...
    def nextframe(self, i=1, imgnum=-1):
        ...
        # draw red x if image is rejected
        if self.reject_index[self.index]:
            self.canvas.coords(self.rejectX_id, (self.canvas.winfo_width()/2, self.canvas.winfo_height()/2))
        else:
            # if no coordinates available, move red circle out of canvas
            self.canvas.coords(self.rejectX_id, (-100, -100))
...
if __name__ == "__main__":
    ...
    # write rejected images to file for later use
    outRejected = open('rejections.dat', 'w')
    numRejected = 0

    for image_idx, image_name in enumerate(app.files):
        if app.reject_index[image_idx]:
            numRejected += 1
            outRejected.write(image_name + "\n")

    print(numRejected, "images tagged for later rejection, list saved to rejections.dat")
